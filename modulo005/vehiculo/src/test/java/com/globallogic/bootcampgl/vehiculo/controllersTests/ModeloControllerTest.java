...
public class ModeloControllerTest {
  ...
  @Mock ModeloService modeloService;

  @InjectMocks ModeloController controller;
  ...
  @Test
  @DisplayName("Received create modelo")
  void test2() {
    ModeloDTO modelo = new ModeloDTO();

    Mockito.doNothing().when(modeloService).createModelo(modelo);

    controller.createModelo(modelo);

    verify(modeloService).createModelo(modelo);
  }
}
